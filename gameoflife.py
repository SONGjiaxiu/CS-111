...
def main():
    ...
    # randomly initializes 50 live cells
    random50 = []
    while len(random50) < 50:
        rand = (random.randrange(1, rows), random.randrange(1, columns))
        if rand not in random50:
            random50.append(rand)
    
    #grid = life(rows, columns, 200, glider, george)
    grid = life(rows, columns, 200, random50, george)
