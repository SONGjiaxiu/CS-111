...
def binarySearch(list, target):
    '''
    inputs: a sorted list of numbers, a target number
    outputs: an index of the location of the target number in the list or
         -1 if the target element is not present in the list.
    preconditions: a list of numbers, sorted in ascending order.
    postconditions: a returned index (or -1), the list is not changed.
 '''   
...
'''
def main ():
    n = 1000         # size of list of numbers
 ...
 '''   spots1 = {}
    ...
    print(spots1)
'''
    # create a dictionary for binary search indices    
    spots2 = {}
    for target in targets:
        index = binarySearch(nums,target)
        if not target in spots2:
            spots2[target] = index
    print(spots2)
